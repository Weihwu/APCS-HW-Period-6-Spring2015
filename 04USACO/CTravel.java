...
public class CTravel{
    ...
    public int getPaths(int r, int c, int t){
        ...
        if (r > pasture.length || r < 1 || c > pasture[r-1].length 
	    || c < 1
	    || pasture[r-1][c-1] == '*' || t == 0){
            return 0;
	}
        return getPaths(r+1, c, t-1) + getPaths(r-1, c, t-1) + getPaths(r, c+1, t-1) + getPaths(r, c-1, t-1);
    }
    ...
    public static void main(String[] args){
        ...
        try{
            Files.write( Paths.get("ctravel.out"),String.valueOf(result).getBytes() );
        }catch(IOException e){
            e.printStackTrace();
        }
    }
